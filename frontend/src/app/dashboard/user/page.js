...
export default function UserDashboard() {
  ...
  const fetchRoleRequests = async () => {
    try {
      const res = await fetch(`${API_URL}/api/user/role-requests`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const json = await res.json();
      if (json.success && Array.isArray(json.data)) {
  ...
  const fetchSavedStartups = async () => {
    try {
      const res = await fetch(`${API_URL}/api/user/saved-startups`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const json = await res.json();
      if (json.success && Array.isArray(json.data)) {
  ...
  const handleWithdraw = async (appId) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/user/applications/${appId}/withdraw`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}` }
      });
      const json = await res.json();
  ...
  if (loading || !user || (user.role !== 'user' && user.role !== 'job_seeker')) {
    return (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pt-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeTab === 'opportunities' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-sm font-black text-slate-955 uppercase tracking-wide flex items-center gap-2">
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-1.5 text-xs font-bold uppercase tracking-wider text-slate-400">
                                <Link href={`/startups/${job.startupId}`} className="hover:underline text-slate-505 hover:text-primary">
                                  {job.startupName}
                                </Link>
                                <span>•</span>
                        ...
                        {job.requiredSkills && job.requiredSkills.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {job.requiredSkills.map((skill, index) => (
                              <span key={index} className="px-2.5 py-1 bg-slate-50 border border-slate-200 text-slate-650 font-bold text-[10px] rounded-lg">
                                {skill}
                              </span>
                            ))}
                          </div>
                      ...
                      <div className="flex items-center gap-2 w-full md:w-auto pt-2 md:pt-0">
                        {['pending', 'reviewed', 'shortlisted'].includes(app.status) && (
                          <button
                            ...
                            className="px-3.5 py-1.5 text-xs border border-red-200 text-red-650 hover:bg-red-50 font-bold rounded-lg transition"
                          >
                            Withdraw
                          </button>
                        )}
                        ...
                        {['connected', 'accepted', 'hired'].includes(app.status) && (
                          <Link
                            href={`/inbox?userId=${app.founderId?._id || app.founderId}`}
            ...
            {activeTab === 'teams' && (
              <div className="space-y-4">
                <h3 className="text-sm font-black text-slate-955 uppercase">My Startup Teams</h3>
                ...
                {loadingTeams ? (
                  ...
                  myTeams.map((team) => (
                    <div key={team._id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition">
                      <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-xl bg-indigo-50 border border-indigo-105 flex items-center justify-center font-bold text-indigo-650 overflow-hidden flex-shrink-0 text-base">
                          {team.startupId?.logo ? (
                      ...
                      <div className="flex gap-2 pt-2 md:pt-0 w-full md:w-auto">
                        <Link
                          href={`/inbox?userId=${team.startupId?.founderId?._id || team.startupId?.founderId}`}
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {savedStartups.map((startup) => (
                      <div key={startup._id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition flex flex-col justify-between h-40">
                        <div>
                          <div className="flex items-center gap-3">
                            <div className="h-10 w-10 bg-blue-100 rounded-xl flex items-center justify-center text-primary font-bold text-lg overflow-hidden border">
                              {startup.logo ? (
                    ...
                    {notifications.map((notif) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <p className="text-xs text-slate-805 font-semibold leading-relaxed">{notif.message || notif.content}</p>
              ...
              <div className="space-y-4 text-xs font-semibold text-slate-600">
                {profile.bio && (
                  <div className="space-y-1">
                    <span className="text-[10px] font-black uppercase tracking-wider text-slate-405">Bio</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-150 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-5 overflow-y-auto pr-2 flex-1">
              <div className="flex flex-wrap gap-4 text-xs font-bold text-slate-605 font-sans">
